...
void *receive_thread(void *arg) {
    ...
    while (1) {
        ...
        len = recvfrom(BACKEND_UDPSOCKET_ID, hdr, DEFAULT_HEADER_LEN, MSG_PEEK,
        ...
        if (len >= DEFAULT_HEADER_LEN) {
            ...
            buf_size = 0;
            while (buf_size <
                   plen) { // 直到接收到 plen 长度的数据 接受的数据全部存在pkt中
                n = recvfrom(BACKEND_UDPSOCKET_ID, pkt + buf_size,
                             plen - buf_size, NO_FLAG,
                             (struct sockaddr *)&from_addr, &from_addr_size);
                buf_size = buf_size + n;
            }
            // 通知内核收到一个完整的TCP报文
...
void startSimulation() {
    ...
    if (bind(BACKEND_UDPSOCKET_ID, (struct sockaddr *)&conn, sizeof(conn)) <
        0) {
    ...
    int rst = pthread_create(&thread_id, NULL, receive_thread,
...
int cal_hash(uint32_t local_ip, uint16_t local_port, uint32_t remote_ip,
