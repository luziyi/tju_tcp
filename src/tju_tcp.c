...
int resent_handing(tju_tcp_t *sock)
{
	...
	{
		uint32_t seq = sock->window.wnd_send->nextseq, ack = 0;
		// uint16_t advertised_wnd = sock->received_capacity - sock->received_len;
...
void handle_loss_ack(tju_tcp_t *sock)
{
	...
	if (sock->window.wnd_send->congestion_status == SLOW_START)
	{
		...
		sock->window.wnd_send->cwnd = 1 * MSS;
...
void handle_valid_ack(tju_tcp_t *sock)
{
	...
	{
		sock->window.wnd_send->cwnd = sock->window.wnd_send->cwnd + (int)(MSS * ((float)MSS / sock->window.wnd_send->cwnd));
...
void handle_dup_ack(tju_tcp_t *sock)
{
	...
	else if (sock->window.wnd_send->congestion_status == CONGESTION_AVOIDANCE && sock->window.wnd_send->ack_cnt == 3)
	{
	...
	else if (sock->window.wnd_send->congestion_status == SLOW_START && sock->window.wnd_send->ack_cnt == 3)
	{
		sock->window.wnd_send->ssthresh = sock->window.wnd_send->cwnd / 2;
		sock->window.wnd_send->cwnd = sock->window.wnd_send->ssthresh + 3 * MSS;
		sock->window.wnd_send->congestion_status = FAST_RECOVERY;
	...
	else if ((sock->window.wnd_send->congestion_status == CONGESTION_AVOIDANCE || sock->window.wnd_send->congestion_status == SLOW_START) && sock->window.wnd_send->ack_cnt < 3)
	{
		return;
	}
	else
